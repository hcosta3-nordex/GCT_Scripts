...
def process_files():
    ...
    for j in range(len(b_parameters)):
        if b_parameters[j] not in matched_params:
            if mode == "NC2-NC2":
                output_tree.insert("", "end", values=(b_parameters[j],parameters_names[j], b_values[j], "Not Present"))
            else:
                output_tree.insert("", "end", values=(b_parameters[j],parameters_names[j], "Not Present", b_values[j]))
