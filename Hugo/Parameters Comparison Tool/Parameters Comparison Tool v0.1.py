...
def process_files():
    ...
    for line in e_lines:
        ...
        if len(elements) > e_index:
            ...
            parameters_names.append(elements[6].strip())
    for line in b_lines:
        elements = line.strip().split(';')
            ...
            if e_parameters[i].lower() == b_parameters[j].lower():
                ...
                try:
                    if float(e_values[i]) != float(b_values[j]):
                        if mode == "NC2-NC2":
                            output_tree.insert("", "end", values=(param_display, b_values[j], e_values[i]))
                        else:
                            output_tree.insert("", "end", values=(param_display, e_values[i], b_values[j]))
                except ValueError:
    ...
    for i in range(len(e_parameters)):
        if e_parameters[i] not in matched_params:
            param_display = f"{e_parameters[i]} - {parameters_names[i]}"
            if mode == "NC2-NC2":
                output_tree.insert("", "end", values=(param_display, "Not Present", e_values[i]))
            else:
                output_tree.insert("", "end", values=(param_display, e_values[i], "Not Present"))
...
root = tk.Tk()
root.title("Parameter Comparison Tool v0.1")
